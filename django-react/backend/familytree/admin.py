...
class FamilytreePersonAdmin(admin.ModelAdmin):
	list_display = ('first_name', 'last_name', 'birth_date', 'status_choices', 'sex_choices', 'birth_place') # fields in Admin Page
	
class FamilytreeRelationshipAdmin(admin.ModelAdmin):
	list_display: ('id_1', 'id_2', 'relationships', 'color', 'title', 'description', 'begin_date', 'end_date', 'descendant') # fields in Admin Page

class FamilytreeMilestoneAdmin(admin.ModelAdmin):
	list_display: ('user_id', 'person_id', 'date', 'title', 'text', 'image') # fields in Admin Page
